...
int main(int argc, char** argv) {
	...
	while (1) {
		...
		// doing this only after a process has run for at least 1 second.
		...
		if (!is_terminated && (current_resource == 0 || (isRequesting() && max_claims > current_resource))) {
			// Make process request some resources by putting request in shared memory.
			// request SHOULD never exceed (the maximum claims - what process already has)
